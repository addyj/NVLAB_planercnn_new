...
def build_targets(p, targets, model):
    ...
    for i, j in enumerate(model.yolo_layers):
        ...
        anchors = model.module.module_list[j-75].anchor_vec if multi_gpu else model.module_list[j-75].anchor_vec
...
def strip_optimizer(f='weights/last.pt'):  # from yolo_utils.utils import *; strip_optimizer()
...
def create_backbone(f='weights/last.pt'):  # from yolo_utils.utils import *; create_backbone()
...
def coco_only_people(path='../coco/labels/train2017/'):  # from yolo_utils.utils import *; coco_only_people()
...
def select_best_evolve(path='evolve*.txt'):  # from yolo_utils.utils import *; select_best_evolve()
...
def crop_images_random(path='../images/', scale=0.50):  # from yolo_utils.utils import *; crop_images_random()
...
def coco_single_class_labels(path='../coco/labels/train2014/', label_class=43):
    # Makes single-class coco datasets. from yolo_utils.utils import *; coco_single_class_labels()
...
def kmean_anchors(path='../coco/train2017.txt', n=12, img_size=(320, 1024), thr=0.10, gen=1000):
    # Creates kmeans anchors for use in *.cfg files: from yolo_utils.utils import *; _ = kmean_anchors()
    # n: number of anchors
...
def plot_wh_methods():  # from yolo_utils.utils import *; plot_wh_methods()
...
def plot_test_txt():  # from yolo_utils.utils import *; plot_test()
...
def plot_targets_txt():  # from yolo_utils.utils import *; plot_targets_txt()
...
def plot_evolution_results(hyp):  # from yolo_utils.utils import *; plot_evolution_results(hyp)
...
def plot_results_overlay(start=0, stop=0):  # from yolo_utils.utils import *; plot_results_overlay()
...
def plot_results(start=0, stop=0, bucket='', id=()):  # from yolo_utils.utils import *; plot_results()
