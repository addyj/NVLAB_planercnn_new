...
from yolo_utils.utils import xyxy2xywh, xywh2xyxy
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, options, config, path, img_size=640, batch_size=5, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        if os.path.exists('./data/customdata/camera.txt'):
            self.camera = np.zeros(6)
            with open('./data/customdata/camera.txt', 'r') as f:
                for line in f:
    ...
    def __getitem__(self, index):
        ...
        if self.mosaic:
            ...
            plane_path = self.plane_files[index]

            depth = np.load(depth_path)
            depth = depth['arr_0'].astype(np.float32) / self.depthShift
            depth = cv2.resize(depth, (self.config.IMAGE_MAX_DIM, self.config.IMAGE_MIN_DIM))

            planes = np.load(plane_path)

            pack_segmentation = np.load(mask_path)
            ...
            image, image_metas, gt_class_ids, gt_boxes, gt_masks, gt_parameters = load_image_gt(self.config, index, img, depth, mask, class_ids, parameters, augment=False)
            ...
            plane_data = [image, image_metas, rpn_match, rpn_bbox, gt_class_ids, gt_boxes, gt_masks, gt_parameters, depth.astype(np.float32), self.extrinsics.astype(np.float32), planes.astype(np.float32), segmentation, plane_indices]
            ...
            plane_data.append(np.concatenate([translation, axis, np.array([angle])], axis=0).astype(np.float32))

            plane_data.append(self.camera.astype(np.float32))
        ...
        return torch.from_numpy(img), labels_out, self.img_files[index], shapes, plane_data
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        img = cv2.resize(img, (int(r), int(r)), interpolation=interp)
...
def reduce_img_size(path='../data/sm4/images', img_size=1024):  # from yolo_utils.datasets import *; reduce_img_size()
...
def convert_images2bmp():  # from yolo_utils.datasets import *; convert_images2bmp()
    # Save images
...
def recursive_dataset2bmp(dataset='../data/sm4_bmp'):  # from yolo_utils.datasets import *; recursive_dataset2bmp()
...
def imagelist2folder(path='data/coco_64img.txt'):  # from yolo_utils.datasets import *; imagelist2folder()
