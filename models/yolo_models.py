...
#Modules edited based on below size of midas out
...
def create_modules(module_defs, img_size):
    ...
    output_filters = [2048]  # input channels
    module_list = nn.ModuleList()
    routs = []  # list of layers which rout to deeper layers
    yolo_index = -1

    for i, mdef in enumerate(module_defs):
        i = i + 75 ## edited cfg model for yolo not darknet
...
class Decoder2(nn.Module):
    ...
    def forward(self, x, midas_c2, midas_c3, augment=False, verbose=False):

        if not augment:
            return self.forward_once(x, midas_c2, midas_c3)
            ...
            for i, xi in enumerate((x,
                ...
                y.append(self.forward_once(xi, midas_c2, midas_c3)[0])
    ...
    def forward_once(self, x, midas_c2, midas_c3, augment=False, verbose=False):
        img_size = x.shape[-2:]  # height, width
        yolo_out, out = [], []

        for out_id in range(75):
            if out_id == 36:
                out.append(midas_c2)
            elif out_id == 61:
                out.append(midas_c3)
...
def get_yolo_layers(model):
    return [i+75 for i, m in enumerate(model.module_list) if m.__class__.__name__ == 'YOLOLayer']  # [89, 101, 113]
